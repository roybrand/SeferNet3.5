CREATE view [dbo].[vGISMapiData]
as
select deptcode, deptName,deptTypeName, 
				deptSubTypeName,
				districtName, cityName, 
				street,
				house, addressComment, zipCode
				, substring (cast( xcoord*1000 + 50000 as varchar) ,1,7) as xcoord
				, substring (cast( ycoord*1000 + 500000 as varchar) ,1,7) as ycoord
				, SiteName, NeighbourhoodName
				, isAutoCoordUpdate, phone, fax , Accessibility , Elevator
 ...
 from 
	(select
				...
				cast(isnull((select top 1 1 from DeptHandicappedFacilities dHF
						...
						, 0) as bit) as Accessibility, 
				cast(isnull((select top 1 1 from DeptHandicappedFacilities dHF
						where dHF.DeptCode = d.deptCode and dhf.FacilityCode = 3)
						, 0) as bit) as Elevator, 
