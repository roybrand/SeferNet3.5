...
alter Procedure [dbo].[rpc_getReportDeptsByTypesProfessionsDynamic]
(
...
begin 
	...
	set @sqlJoin = @sqlJoin +  ' inner join dept as dAdmin on d.administrationCode = dAdmin.deptCode  ' ;
...
begin 
	...
	set @sqlJoin = @sqlJoin +  ' inner join dept as dDistrict on d.districtCode = dDistrict.deptCode ' ;
...
--	end 
	...
	set @sqlJoin = @sqlJoin +  ' inner join DeptStatus on d.DeptCode = DeptStatus.DeptCode '+ @NewLineChar + 
...
begin 
	...
	set @sql = @sql + ' case 
		...
		end as MangerName '+ @NewLineChar;

	set @sqlJoin = @sqlJoin +  ' left join x_Dept_Employee_Position as tblDeptManager on d.DeptCode = tblDeptManager.deptCode and tblDeptManager.PositionCode = 13 ' + @NewLineChar +
							   ' left join Employee as tblEmpManager on  tblDeptManager.EmployeeID = tblEmpManager.EmployeeID '+ @NewLineChar;
end 
---------AdminMangerName---------------------------------------------------------------
...
begin 
	...
	set @sql = @sql + ' case 
			...
			end as AdminMangerName '+ @NewLineChar;

	set @sqlJoin = @sqlJoin +  ' left join x_Dept_Employee_Position as tblAdminDeptManager on d.DeptCode = tblAdminDeptManager.deptCode and tblDeptManager.PositionCode = 5 ' + @NewLineChar +
							   ' left join Employee as tblAdminEmpManager on  tblAdminDeptManager.EmployeeID = tblAdminEmpManager.EmployeeID '+ @NewLineChar;
end 
...
begin 
	...
	set @sql = @sql + ' x_Dept_Employee_Service.serviceCode	as EmployeeServiceCode,		
		(select serviceDescription 
			from service 
			where service.serviceCode = x_Dept_Employee_Service.serviceCode) as EmployeeServices '+ @NewLineChar;
...
--
set @sql = @sql + ' if (' +  cast( @PageSise as varchar(20)) + ' > -1) ' + @NewLineChar +
	' begin ' + @NewLineChar + 
		' SELECT TOP (' + cast( @PageSise as varchar(20)) + ') * FROM #dd3 ' + @NewLineChar + 
		' WHERE  RowNumber >' +  cast(@StartingPosition as varchar(20)) + ' AND ' +' RowNumber <= ' + cast(@StartingPosition as varchar(20)) + '+ ' + cast( @PageSise as varchar(20))  + @NewLineChar + 
		' ORDER BY RowNumber ' + @NewLineChar + 
		' select count(*) from #dd3 ' + @NewLineChar + 
		' END ' + @NewLineChar +
		' if (' +  cast(@StartingPosition as varchar(20)) + ' = -1 )' + @NewLineChar + 
			' begin ' + @NewLineChar + 
			' SELECT * FROM #dd3 ' + @NewLineChar + 
			' ORDER BY RowNumber	' + @NewLineChar + 
		' end ' + @NewLineChar +
...
GRANT EXEC ON rpc_getReportDeptsByTypesProfessionsDynamic TO PUBLIC
