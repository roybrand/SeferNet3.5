IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'rpc_getDeptsByTypesProfessions_PagedSorted')
	BEGIN
		DROP  Procedure  rpc_getDeptsByTypesProfessions_PagedSorted
	END

GO

create Procedure [dbo].[rpc_getDeptsByTypesProfessions_PagedSorted]
(
...
--IF EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='dd3') DROP TABLE TABLE_NAME 

--SELECT TOP (@PageSise) * FROM
...
(
	Select	distinct d.deptCode as ClinicCode ,						 d.DeptName as ClinicName,  
		...
		case 
			when DHC1.FacilityCode is not null then 'yes'
			else 'no'
		end as AccessToClinic, 
		case 
			when DHC2.FacilityCode is not null then 'yes'
			else 'no'
		end as DisabledServices, 
		case 
			when DHC3.FacilityCode is not null then 'yes'
			else 'no'
		end as ElevatorOrOneFloorStructure, 
		case 
			when DHC4.FacilityCode is not null then 'yes'
			else 'no'
		end as ElectricBed, 
		...
		'professionDescription' = dbo.rfn_GetDeptProfessionsCodes_New(d.DeptCode) ,
		...
		dbo.fun_GetDeptServices(d.DeptCode) as DeptServices 
		/*(select serviceDescription 
			from service 
			where service.serviceCode = x_dept_Service.serviceCode) as DeptsServices*/
		--'DeptsServices' = dbo.rfn_GetDeptServices
		...
		left join x_Dept_Employee_Position as tblAdminDeptManager on d.DeptCode = tblAdminDeptManager.deptCode and tblAdminDeptManager.PositionCode = 5
		...
		--INNER JOIN service as service	ON x_dept_Service.serviceCode = service.serviceCode
...
begin 	
	SELECT TOP (@PageSise) * FROM #dd3
	WHERE 
	RowNumber > @StartingPosition
	AND 
	RowNumber <= @StartingPosition + @PageSise
	ORDER BY RowNumber
	select count(*) from #dd3
...
GRANT EXEC ON rpc_getDeptsByTypesProfessions_PagedSorted TO PUBLIC
