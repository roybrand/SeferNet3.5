...
<%@ Page Title="עדכון הערות לעובד" Language="C#" MasterPageFile="~/SeferMasterPageIEwide.master"
    AutoEventWireup="true" Inherits="UpdateEmployeeRemarks"
    UICulture="he-il" Culture="he-il" Codebehind="UpdateEmployeeRemarks.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="pageContent" runat="Server">
    ...
    <script language="jscript" type="text/jscript">
        ...
        function OpenEmployeeDepts(imgCtrl) {

            document.getElementById('<%=txtImgButtonClientID.ClientID %>').value = imgCtrl.id;

            var hdnRemarkID = $('#' + imgCtrl.id).closest("tr").find("[id$='hdnRemarkID']").attr('id');
            var txtEmployeeDepts = $('#' + imgCtrl.id).closest("tr").find("textarea[id$='txtEmployeeDepts']").attr('id');
            var hdnValues = $('#' + imgCtrl.id).closest("tr").find("[id$='hdnValues']").attr('id');

            remarkID = $('#' + imgCtrl.id).closest("tr").find("[id$='hdnRemarkID']").val();

            selectedCodes = $('#' + imgCtrl.id).closest("tr").find("[id$='hdnValues']").val();

            textBoxValue = $('#' + imgCtrl.id).closest("tr").find("textarea[id$='txtEmployeeDepts']").val();

            url = "SelectEmployeeDeptsPopup.aspx?EmployeeRemarkID=" + remarkID;

            if (selectedCodes != '' && textBoxValue != '') {
                url += "&selectedCodes=" + selectedCodes;
            }

            url += "&returnValuesTo=" + hdnValues;
            url += "&returnTextTo=" + txtEmployeeDepts;
        ...
        function SetSelectedValues(codes, text) {
            alert(codes);
<%--            var imgCtrl_ID = document.getElementById('<%=txtImgButtonClientID.ClientID %>').value;
            if (imgCtrl_ID != "") {
                $('#' + imgCtrl_ID).closest("tr").find("textarea[id$='txtEmployeeDepts']").val(obj.Text);

                $('#' + imgCtrl_ID).closest("tr").find("input[id$='hdnValues']").val(obj.Value);
            }--%>
        }

        function SetSelected(source, eventArgs) {

            $('#' + source._id.replace('acDepts', 'hdnValues')).val(eventArgs._value);
    ...
    <table id="AllScreen" border="0" width="985px">
        <tr>
            <td>
                <table cellpadding="0" cellspacing="0" style="height: 30px; margin-right: 10px; margin-top: 15px"
                    border="0" dir="rtl">
                    <tr>
                        <td>
                            <span style="vertical-align: text-top" class="RegularLabel">יחידה:</span>
                            <asp:TextBox ID="txtImgButtonClientID" runat="server" EnableTheming="false" CssClass="DisplayNone"></asp:TextBox>
                        </td>
                        <td>
                            <asp:DropDownList ID="ddlEmployeeDepts" Width="150px" runat="server">
                            </asp:DropDownList>
                        </td>
                        <td>
                            <table cellpadding="0" cellspacing="0" style="margin-bottom: 2px">
                                <tr>
                                    ...
                                    <td style="vertical-align: middle; background-image: url(../Images/Applic/regButtonBG_Middle.gif);
                                        background-repeat: repeat-x; background-position: center;">
                                        <asp:Button ID="btnFilterByArea" runat="server" Text="סינון" CssClass="RegularUpdateButton"
                                            Width="50px" OnClick="FilterRemarks" />
                                    </td>
                                    <td style="background-position: right; background-image: url(../Images/Applic/regButtonBG_Left.gif);
        ...
        <tr id="trGrids">
            <td id="tdGrids">
                <div style="height: 390px; overflow: auto; padding: 2px; direction: ltr" class="ScrollBarDiv">
                                    ...
                                    <tr>
                                        <td>
                                            <asp:GridView ID="gvCurrentRemarks" runat="server" Width="100%" AutoGenerateColumns="false" ShowHeader="False"
                                                OnRowDataBound="gvCurrentRemarks_RowDataBound" HeaderStyle-Height="0" RowStyle-VerticalAlign="Top"
                                                ...
                                                <Columns>
                                                                ...
                                                                <tr>
                                                                    ...
                                                                    <div style="width:780px; white-space:nowrap">
                                                                        ...
                                                                        <div style="float:right; padding-right:10px; width:160px">
                                                                            ...
                                                                            <asp:TextBox ID="txtEmployeeDepts" runat="server" Height="30px" Width="130px" Rows="1"
                                                                                TextMode="MultiLine" CssClass="TextBoxMultiLine"></asp:TextBox>
                                                                            <ajaxToolkit:AutoCompleteExtender ID="acDepts" runat="server" TargetControlID="txtEmployeeDepts"
                                                                                ServicePath="~/AjaxWebServices/AutoComplete.asmx" ServiceMethod="GetEmployeeDepts"
                                                                                ...
                                                                                CompletionListCssClass="CopmletionListStyleWidth" OnClientItemSelected="SetSelected" />
           
                                                                                <asp:ImageButton ID="btnRoles" runat="server" ImageUrl="../Images/applic/icon_magnify.gif"
                                                                                    OnClientClick="OpenEmployeeDepts(this); return false;" />
                                                    ...
                                                    <asp:BoundField DataField="DeptsCodes" ItemStyle-Width="0px">
                                                        <ItemStyle CssClass="DisplayNone" />
                                                        <HeaderStyle CssClass="DisplayNone" />
                                                    </asp:BoundField>
                                                </Columns>
                                    ...
                                    <tr>
                                        <td>
                                            <asp:GridView ID="gvFutureRemarks" runat="server" Width="100%" AutoGenerateColumns="false" ShowHeader="False"
                                                OnRowDataBound="gvFutureRemarks_RowDataBound" RowStyle-VerticalAlign="Top" HeaderStyle-Height="0"
                                                EnableViewState="true" SkinID="GridViewForUpdatePage" HeaderStyle-CssClass="GridViewColumnHeader">
                                                <Columns>
                                                    <asp:TemplateField HeaderText="" Visible="false">
                                                        <ItemTemplate>
                                ...
                                <asp:GridView ID="gvHistoricRemarks" runat="server" Width="100%" AutoGenerateColumns="false"
                                    ...
                                    <Columns>
                                        ...
                                        <asp:TemplateField HeaderText="ביחידה">
                                            <ItemTemplate>
                                                <asp:HiddenField ID="hdnRemarkID" runat="server" Value='<%# Eval("RemarkID") %>' />
                                                <asp:HiddenField ID="hdnValues" runat="server"  Value='<%# Eval("DeptsCodes") %>' />
                                                <asp:TextBox ID="txtEmployeeDepts" runat="server" Height="30px" Width="210px" Rows="1" Enabled="false"
                                                    TextMode="MultiLine" CssClass="TextBoxMultiLine" Text='<%# Eval("AreasNames") %>'></asp:TextBox>
                                            </ItemTemplate>
                                        </asp:TemplateField>
